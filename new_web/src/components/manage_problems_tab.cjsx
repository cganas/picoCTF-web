...
CollapsibleMixin = require 'react-collapsible-mixin'
...
CollapsibleInformation = React.createClass
  ...
  render: ->
    ...
    <Panel header={header} collapsible expanded={@state.expanded}>
        {this.props.children}
    </Panel>
...
Problem = React.createClass
  ...
  onStateToggle: (e) ->
    e.preventDefault()
    Api.call "POST", "/api/admin/problems/availability", {pid: @props.pid, state: !@props.disabled}
...
ProblemDependencyView = React.createClass
  handleClick: (bundle) ->
    Api.call "POST", "/api/admin/bundle/dependencies_active", {bid: bundle.bid, state: !bundle.dependencies_enabled}
    .done @props.onProblemChange
